...
**Layer 3 (§2.4) confirmed:** the cron-job.org GitHub PAT had expired. Both jobs were restored with a classic PAT carrying the `repo` scope. Phase 3.4's earlier `Contents: read` guidance was wrong: GitHub documents fine-grained PAT support for `repository_dispatch` with `Contents: write`, and a classic PAT with `repo` scope remains a valid fallback. A missing `Bearer ` prefix produces the same 401 as an expired token.
