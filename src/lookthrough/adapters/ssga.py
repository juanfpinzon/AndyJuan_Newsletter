...
class SsgaAdapter(BaseAdapter):
    issuer = "ssga"
    base_url = "https://www.ssga.com/etfs"
    ...
    async def fetch(self, etf_id: str) -> list[Holding]:
        ...
        try:
            ...
            raise LookthroughFailure(
                f"Unable to fetch SSgA holdings for {etf_id}",
                issuer=self.issuer,
                etf_id=etf_id,
            ) from exc
        ...
        if not holdings:
            raise LookthroughFailure(
                f"Unable to parse SSgA holdings for {etf_id}",
                issuer=self.issuer,
                etf_id=etf_id,
            )
        return holdings[:10]
